...
| Template | Best For |
|----------|----------|
| [A — RTF](#template-a--rtf) | Simple one-shot tasks in Claude chat |
...
| [E — Chain of Thought](#template-e--chain-of-thought) | Logic, math, analysis, debugging (standard Claude only) |
| [F — Few-Shot](#template-f--few-shot) | Consistent structured output, pattern replication |
| [H — ReAct + Stop Conditions](#template-h--react--stop-conditions) | Claude Code and Claude Cowork — autonomous agents |
...
*Use for logic-heavy tasks, math, debugging, and multi-factor analysis where Claude needs to reason carefully before committing to an answer.*

**Important:** Only use CoT for standard Claude chat. Do NOT add CoT instructions to Claude extended thinking mode — it reasons internally and CoT instructions degrade output.
...
**When NOT to use:**
- Claude extended thinking mode (thinks internally — adding CoT hurts)
...
*Use for Claude Code and Claude Cowork — any Claude session that takes autonomous actions. Runaway loops and scope explosion are the biggest credit killers in agentic workflows — stop conditions are not optional.*
